...
import { ApprovalReflectionPayload } from './domain-events';
...
export class DomainEventBus {
  ...
  publish(event: string, payload: ApprovalReflectionPayload): void {
    this.emitter.emit(event, payload);
  }

  subscribe(
    event: string,
    handler: (payload: ApprovalReflectionPayload) => void | Promise<void>,
  ): void {
    // 핸들러 내부에서 자체적으로 예외를 처리한다(구독자 실패가 다른 구독자·발행자에 전파되지 않도록)
    this.emitter.on(event, (payload: ApprovalReflectionPayload) => {
