...
// 최초 진입 시 GET /auth/me로 확인하고, 이후에는 모듈 캐시를 재사용한다.
// API 401 응답(mutator) 또는 로그아웃 시 캐시를 비운다.
...
/** 보호 라우트 beforeLoad 통과 후에는 항상 non-null — app-shell 등 렌더 단계에서 사용 */
...
/** 보호 라우트 공통 beforeLoad — 미로그인 시 /login으로 보낸다 */
...
/** /login 전용 beforeLoad — 이미 로그인돼 있으면 대시보드로 보낸다 */
